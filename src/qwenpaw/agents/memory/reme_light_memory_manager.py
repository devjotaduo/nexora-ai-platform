...
class ReMeLightMemoryManager(BaseMemoryManager):
    ...
    def __init__(self, working_dir: str, agent_id: str):
        ...
        for tool_name, tool_func in {
            ...
            if self._agent_can_use_summary_tool(tool_name):
                self.summary_toolkit.register_tool_function(tool_func)
            else:
                logger.debug(
                    "Skipped unauthorized memory summary tool for agent %s: %s",
                    self.agent_id,
                    tool_name,
                )

    def _agent_can_use_summary_tool(self, tool_name: str) -> bool:
        from qwenpaw_ext.nexora.governance import agent_can_use_resource

        return agent_can_use_resource(self.agent_id, "builtin_tool", tool_name)
